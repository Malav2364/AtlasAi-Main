...
const TimeLineScroll = () => {
    ...
    const ctx = gsap.context(() => {
      if (
        ...
        tl
          ...
          .to(video1El, { opacity: 1, scale: 1, duration: 1, ease: "power2.out", zIndex: 1 }, "<")
          .to(video1El, { scale: 1.1, x: '-2%', duration: 10, ease: "none" }, "<")
          
          .to(text1Chars, { opacity: 0, yPercent: 50, stagger: 0.05, duration: 0.5 }, "+=0.5")
          .to(video1El, { opacity: 0, scale: 0.8, duration: 0.5, ease: "power1.in" }, "<") 
          .fromTo(video2El, 
              { opacity: 0, scale: 0.1, zIndex: 1 },
              { opacity: 1, scale: 1, duration: 1, ease: "power2.out" },
              "<"
          )
          .to(video2El, { scale: 1.1, x: '2%', duration: 10, ease: "none" }, "<")
          .from(text2Chars, {
              opacity: 0, y: 50, filter: 'blur(10px)',
              stagger: { each: 0.05, from: "random" },
              duration: 1, ease: "power2.out"
          ...
          .to(text2Chars, { opacity: 0, yPercent: 50, stagger: 0.05, duration: 0.5 }, "+=0.5")
          .to(video2El, { opacity: 0, scale: 0.8, duration: 0.5, ease: "power1.in" }, "<")
          .fromTo(video3El, 
              { opacity: 0, scale: 0.1, zIndex: 1 },
              { opacity: 1, scale: 1, duration: 1, ease: "power2.out" },
              "<"
          )
          .to(video3El, { scale: 1.1, y: '-2%', duration: 10, ease: "none" }, "<")
          .from(text3Chars, {
              opacity: 0, y: 50, filter: 'blur(10px)',
              stagger: { each: 0.05, from: "random" },
              duration: 1, ease: "power2.out"
  ...
  return (
    <section ref={sectionRef} className="relative h-screen bg-white text-black overflow-hidden">
      {/* UPDATED: Stable video links from Pixabay */}
      <video 
          ref={video1Ref}
          src="travel.mp4"
      ...
      <div className="absolute inset-0 w-full h-full flex items-center justify-center z-10">
        ...
        <AnimatedText ref={text2Ref} text="Explore." className="absolute text-[20vw] font-extrabold text-black select-none z-20 blend-difference" style={{ lineHeight: '1' }} />
